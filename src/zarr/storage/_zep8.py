    ...
    def extract_path(self, url: str) -> str:
        ...
        for segment in reversed(segments):
            ...
            if segment.adapter in ("zarr2", "zarr3") and segment.path and not zarr_path:
                ...
                # Special handling for icechunk: paths like "branch:main", "tag:v1.0", "snapshot:abc123"
                # are metadata, not zarr paths
                if (
                    segment.adapter in ("icechunk", "ic")
                    and ":" in segment.path
                    and segment.path.split(":")[0] in ("branch", "tag", "snapshot")
                ):
