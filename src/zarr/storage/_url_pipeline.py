...
This module is imported lazily from `zarr.storage._common` — importing zarr
does not import it, and no adapters are loaded until a pipeline URL is
actually resolved.
"""
...
def is_url_pipeline(url: str) -> bool:
    ...
    if "|" in url:
        return True
    scheme = parse_store_url(url).scheme.lower()
...
async def resolve_pipeline(
    ...
    *,
    ...
    """
    Resolve a URL pipeline into a store, residual path, and optional
    zarr format.
