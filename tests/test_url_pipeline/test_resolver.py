...
class TracingStore(WrapperStore[MemoryStore]):
...
class TestResolve:
    async def test_wrapper_adapter_chain(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        result = await resolve_pipeline("memory://base|wrap:inner/path")
        assert isinstance(result.store, TracingStore)
        assert result.path == "inner/path"
        assert result.store.context.preceding_url == "memory://base"
        assert result.zarr_format is None
    ...
    async def test_read_only_flag(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        result = await resolve_pipeline("memory://base|wrap:", mode="r")
        assert result.store.context.read_only
        assert result.store.context.mode == "r"
        result = await resolve_pipeline("memory://base|wrap:")
...
class TestMakeStoreIntegration:
    async def test_make_store_path_combines_paths(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        store_path = await make_store_path("memory://base|wrap:residual", path="user/sub")
        assert store_path.path == "residual/user/sub"
        assert store_path.zarr_format is None

    async def test_make_store_rejects_residual_path(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        with pytest.raises(URLPipelineError, match="resolves to a path inside a store"):
            await make_store("memory://base|wrap:residual")

    async def test_make_store_no_residual_path(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        store = await make_store("memory://base|wrap:")
        assert isinstance(store, TracingStore)

    async def test_zarr_open_end_to_end(self) -> None:
        register_url_adapter("wrap", WrapperAdapter)
        group = zarr.open_group("memory://base|wrap:", mode="w")
        array = group.create_array("x", shape=(4,), dtype="i4")
        array[:] = [1, 2, 3, 4]
        assert zarr.open_array("memory://base|wrap:x")[2] == 3
